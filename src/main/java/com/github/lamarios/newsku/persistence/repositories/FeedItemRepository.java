...
public interface FeedItemRepository extends JpaRepository<FeedItem, String> {
    ...
    @Query(value = "select i from FeedItem i where i.feed in :feeds and i.importance >= :minImportance and i."
            + "timeCreated > :from and i.timeCreated <= :to")
    Page<FeedItem> findallByTimeAndFeeds(
            @Param("minImportance") int minImportance,
            @Param("from") long from,
            @Param("to") long to,
            @Param("feeds") List<Feed> feeds,
            Pageable pageable
    );
