                ...
                ...feeds.map((f) {
                  return ListTile(
                    leading: Draggable<Feed>(
                      ...
                      onDragStarted: cubit.dragStarted,
                      onDragEnd: cubit.dragEnded,
                      onDragCompleted: () => cubit.dragEnded(null),
                      onDraggableCanceled: (velocity, offset) => cubit.dragEnded(null),
                      data: f,
...
class FeedCategoryDragTarget extends StatelessWidget {
  final FeedCategory category;

  const FeedCategoryDragTarget({super.key, required this.category});
      ...
      builder: (context, List<Feed?> candidates, List<dynamic> rejected) {
        return Padding(
          ...
          child: SingleMotionBuilder(
            from: 0,
            value: candidates.isNotEmpty ? 1 : 0,
            motion: MaterialSpringMotion.expressiveSpatialDefault(),
            builder: (context, value, child) => Container(
              padding: EdgeInsets.all(pu2),
              margin: .only(bottom: pu2, left: pu, right: pu),
              decoration: BoxDecoration(
                borderRadius: .circular(pu3),
                border: Border.all(color: colors.primary),
                color: Color.lerp(colors.surface, colors.primary, value) ?? colors.surface,
              ),
              child: child,
            ),
            child: Text(category.id == null ? locals.uncategorized : category.name, style: textTheme.bodyLarge),
          ),
        );
      },
    );
